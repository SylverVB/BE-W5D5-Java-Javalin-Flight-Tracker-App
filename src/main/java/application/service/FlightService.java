...
public class FlightService {
    ...
    public Flight updateFlight(int flightId, Flight flight) {
        // Check if the flight with the provided ID exists
        Flight existingFlight = flightDAO.getFlightById(flightId);
    
        // If the flight exists, proceed with the update
        if (existingFlight != null) {
            // Call the DAO method to update the flight
            flightDAO.updateFlight(flightId, flight);
    
            // Assuming the updated flight has the same flightId, we can re-fetch it if needed
    ...
    /**
     * Use the FlightDAO to retrieve a List containing all flights departing from a certain city and arriving at
